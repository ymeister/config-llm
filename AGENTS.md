...
Apply when the user gives the same correction more than once, states a preference in passing, or the work shows a convention here to be wrong, ambiguous, or incomplete.
...
Apply when you need a nix store path or the source of a nix dependency.
...
## Questions during approved work
...
Apply when you write new code, add a definition, or move code within a file.
...
Apply when you write a code comment, a doc comment, an option or field description, or a test name.
...
- Write ordered facts, such as a precedence, a fallback chain, or a priority ladder, as a numbered list. The order is part of the content.
...
Apply when you write or edit Haskell, even when the code around it differs from these rules.
